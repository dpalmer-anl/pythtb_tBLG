...
def make_layer(stacking,cell_type,n_1,n_2,lat_con,z_val,sym,mass,mol_id):
    ...
    if (cell_type=='rect'):
        ...
        layer=fact(directions=[[1,0,0],[0,1,0],[0,0,1]],
                   ...
                   symbol=['Ar','B']) #dummy_sym
        
        sublat = layer.get_chemical_symbols()
        # sublat[np.where(sublat=='Ar')] = 'A'
        layer.sublattice = sublat
...
if __name__=="__main__":
    ...
    n_layer = 2
    sep = 3.35
    atoms = make_layer('A','hex',1,1,lat_con,1.0,'C',12.01,1)
    print(atoms.sublattice)